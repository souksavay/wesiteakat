...
if(isset($_POST["btnSaveNew"]) && $_SESSION['EGPOSAKATrole_id'] <= 2) {
	...
	if($tmp_file_name != "") { 
		...
		copy($_FILES["txtImage"]["tmp_name"],"../img/faq/".$file_name);

		$fullPath = "img/faq/".$file_name; 
	}

	$sql = "INSERT INTO tb_faq (faqTitle, faqDescript, topNo, faqImg, status_id, titleType, langID, user_add, date_add)
			VALUES ('$txtTitle', '$txtDescription', '$txtTopno', '$fullPath', '$cbActive', '$cbHeader', '$lang', '$GET_userID', NOW()) ";	 
	if ($mysqli->query($sql) === TRUE) {
		header("Location: ?d=module/faq");
	}else{
...
if(isset($_POST["btSaveEdit"]) && $_SESSION['EGPOSAKATrole_id'] <= 2) {
	...
	if($tmp_file_name != "") { 
		...
		copy($_FILES["txtImage"]["tmp_name"],"../img/faq/".$file_name);
		$fullPath = "img/faq/".$file_name; 		
		$sql_img = "UPDATE tb_faqs SET faqImg='$fullPath' WHERE faqID = '$txtId' ";	 
		$mysqli->query($sql_img); 
	}

	$sql = "UPDATE tb_faq SET faqTitle='$txtTitle', faqDescript='$txtDescription', topNo='$txtTopno', status_id='$cbActive', titleType='$cbHeader', langID='$lang' , user_add='$GET_userID', date_add=NOW() WHERE faqID = '$txtId' ";	 	
	if ($mysqli->query($sql) === TRUE) {
		if ($cbHeader == "1") {
			$sql_h = "UPDATE tb_faq SET titleType = 0 WHERE langID = '$lang' and faqID <> '$txtId'";	
			$mysqli->query($sql_h); 
		} 
...
if(isset($_GET["faq"]) && isset($_GET["header"]) && isset($_GET["lang"]) && $_SESSION['EGPOSAKATrole_id'] <= 2){	
	$id = $mysqli->real_escape_string(base64_decode($_GET['faq'])); 	 
	...
	if ($headerID == "1") {
		...
		$sql = "UPDATE tb_faqs SET titleType = 0 WHERE faqID ='$id'";	 
		if ($mysqli->query($sql) === TRUE) {
			header("Location: ?d=module/faq");
		}else{
		...
		$sql = "UPDATE tb_faq SET titleType = 1 WHERE faqID ='$id'";	 

		
		if ($mysqli->query($sql) === TRUE) {
			$sql = "UPDATE tb_faq SET titleType = 0 WHERE langID = '$langID' and faqID <> '$id'";	 
			if ($mysqli->query($sql) === TRUE) {
				header("Location: ?d=module/faq");
			} else {
				echo "<center><h2>ERROR UPDATE</h2></center>";
...
if(isset($_GET["faq"]) && isset($_GET["active"]) && isset($_GET["lang"]) && $_SESSION['EGPOSAKATrole_id'] <= 2){	
	$id = $mysqli->real_escape_string(base64_decode($_GET['faq'])); 	 
	$id = substr($id, 0, strlen($id)-11);
	...
	$sql = "UPDATE tb_faq SET status_id = '$activeID' WHERE faqID ='$id'";	 
	if ($mysqli->query($sql) === TRUE) {
		header("Location: ?d=module/faq");
	}else{
		echo "<center><h2>ERROR UPDATE</h2></center>";
...
if(isset($_GET["faq"]) && isset($_GET["del"]) && isset($_GET["lang"]) && $_SESSION['EGPOSAKATrole_id'] <= 2){	
	$id = $mysqli->real_escape_string(base64_decode($_GET['faq'])); 	 
	$id = substr($id, 0, strlen($id)-11);	 		 

	$sql = "UPDATE tb_faq SET status_id = '3' WHERE faqID ='$id'";	 
	if ($mysqli->query($sql) === TRUE) {
		header("Location: ?d=module/faq");
	}else{
		echo "<center><h2>ERROR UPDATE</h2></center>";
