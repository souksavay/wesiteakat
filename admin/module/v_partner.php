                ...
                <div class="modal-dialog modal-lg">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h4 class="modal-title">Partner Item</h4>
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                      </button>
                    </div>
                    <form method = "post" action="?d=module/partner" enctype="multipart/form-data">
                          ...
                          <div class="col-sm-6">
                            <div class="form-group">
                              ...
                              <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="cbHeader" value="1" >
              ...
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <th>Description</th>
                    <th>Images</th>
                    <th width="5%" align="center">Header</th>                    
                  ...
                  <tbody>
                   <?php
                   $i=1;
                    if ($result = $mysqli->query($qr_partner)) {
                        while ($row = $result->fetch_row()) {
                          $partnerID = base64_encode($row[0]."tasoksavhay");
                          $headerVal = base64_encode($row[7]."tasoksavhay");
                          $activeVal = base64_encode($row[5]."tasoksavhay");
                          $GetLang = base64_encode($row[8]."tasoksavhay");
                   ...
                   ?>
                                  ...
                                  <div class="form-group">
                                    ...
                                    <select class="form-control" name="txtLang">
                                      <?php
                                        if ($result_lang = $mysqli->query($qr_Lang)) {
                                          while ($row_lan = $result_lang->fetch_row()) {
                                            $selected = $row_lan[0] == $row[8] ? "selected" : ""; 
                                      ?>
                                ...
                                <div class="col-sm-6">
                                  <div class="form-group">
                                    ...
                                    <div class="form-check">
                                      ...
                                      ?>
                                      <input class="form-check-input" type="checkbox" name="cbHeader" <?=$headerCheck?> value="1" >
                                      <label class="form-check-label">Header</label>
                                    </div>                        
                                  </div>
                                </div>
                    ...
                    <tr>
                      ...
                      <td align="center">
                        <?php if ($row[7] == "1") {?>
                          <a href ="?d=module/partner&partner=<?=$partnerID?>&header=<?=$headerVal ?>&lang=<?=$GetLang?>" ><i class="far fa-check-square"></i></a>
                        <?php } else { ?>
                          <a href ="?d=module/partner&partner=<?=$partnerID?>&header=<?=$headerVal ?>&lang=<?=$GetLang?>" ><i class="fal fa-square-full"></i></a>
                        <?php } ?>
                      </td>                      
                      <td align="center">
                        <?php if ($row[5] == "1") {?>
                          <a href ="?d=module/partner&partner=<?=$partnerID?>&active=<?=$activeVal ?>&lang=<?=$GetLang?>" ><i class="far fa-check-square"></i></a>
                        <?php } else { ?>
                          <a href ="?d=module/partner&partner=<?=$partnerID?>&active=<?=$activeVal ?>&lang=<?=$GetLang?>" ><i class="fal fa-square-full"></i></a>
                        <?php } ?>
                      </td>
                      <td align="center"><a href="#"><i class="fas fa-edit" data-toggle="modal" data-target="#modal-lg-Edit<?=$i?>"></i></a></td>
                      <td align="center">
                        <a href ="?d=module/partner&partner=<?=$partnerID?>&del=<?=$activeVal ?>&lang=<?=$GetLang?>" onclick="return confirm('ທ່ານຕ້ອງການລຶບແທ້ບໍ...?')" ><i class="far fa-trash-alt"></i></a>
                      </td>
                    </tr>
                  <?php $i++; } } ?>
                  </tbody>
